...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'GL_SVDMSE_HER',
                      "vision_depth": 0,
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.n_ctx_global = cfg.TRAINER.GL_SVDMSE_HER.N_CTX_GLOBAL
        max_prompt_length = max(self.n_ctx_global, max(self.user_prompt_lengths))

        ctx_init = cfg.TRAINER.GL_SVDMSE_HER.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        self.N = cfg.TRAINER.GL_SVDMSE_HER.N
        self.ratio = cfg.TRAINER.GL_SVDMSE_HER.ratio
        self.num_experts = cfg.TRAINER.GL_SVDMSE_HER.NUM_EXPERTS
        assert cfg_imsize == clip_imsize, f"cfg_imsize ({cfg_imsize}) must equal to clip_imsize ({clip_imsize})"

        if cfg.TRAINER.GL_SVDMSE_HER.CSC:
            raise NotImplementedError(
                "GL_SVDMSE_HER does not support CSC when clients use different prompt lengths. Set CSC=False."
            )
        ...
        if ctx_init:
            ...
            init_ctx_vectors = embedding[0, 1:-1, :].clone()

            def build_init_ctx(num_ctx):
                if init_ctx_vectors.size(0) >= num_ctx:
                    return init_ctx_vectors[:num_ctx, :].clone()

                extra_ctx = torch.empty(num_ctx - init_ctx_vectors.size(0), ctx_dim, dtype=dtype)
                nn.init.normal_(extra_ctx, std=0.02)
                return torch.cat([init_ctx_vectors, extra_ctx], dim=0)
            
            self.ctx_global = nn.Parameter(build_init_ctx(self.n_ctx_global))

            self.ctx_local_experts = nn.ParameterList([
                nn.Parameter(
                    torch.stack(
                        [build_init_ctx(prompt_len) for _ in range(self.num_experts)],
                        dim=0
                    )
                )
                for prompt_len in self.user_prompt_lengths
            ])

            self.router_logits_list = nn.ParameterList([
                nn.Parameter(torch.zeros(self.num_experts, dtype=dtype))
            ...
            self.ctx_local_experts = nn.ParameterList([
                nn.Parameter(torch.empty(self.num_experts, self.user_prompt_lengths[idx], ctx_dim, dtype=dtype))
            ...
            for param in self.ctx_local_experts:
    ...
    def forward(self, idx):
        ...
        for i, param in enumerate(self.ctx_local_experts):
        ...
        prompts = self.compose_prompts(ctx_local, prefix, suffix, n_ctx_local)
        prompts_global = self.compose_prompts(ctx_global, prefix, suffix, n_ctx_global)
        prompts_projected_local = self.compose_prompts(projected_ctx_local, prefix, suffix, n_ctx_local)
    ...
    def compose_prompts(self, ctx, prefix, suffix, n_ctx):
        ...
        prompts = []
        total_prompts = ctx.size(0)

        for i in range(total_prompts):
            name_len = self.name_lens[i % self.n_cls]
    ...
    def init_experts_from_global(self):
        with torch.no_grad():
            global_ctx = self.ctx_global.detach().to(torch.float32)   # [L_g, D]
            _, S, V = torch.svd(global_ctx)

            rank = V.shape[1]
            seg_size = max(1, rank // self.num_experts)

            for user_idx, expert_tensor in enumerate(self.ctx_local_experts):
                local_len = expert_tensor.shape[1]

                for m in range(self.num_experts):
                    start = m * seg_size
                    end = rank if m == self.num_experts - 1 else min(rank, (m + 1) * seg_size)

                    basis = V[:, start:end]   # [D, r_m]

                    if basis.shape[1] == 0:
                        nn.init.normal_(expert_tensor[m], std=0.02)
                        continue
...
class GL_SVDMSE_HER(TrainerX):
    ...
    def build_model(self):
        cfg = self.cfg
        self.lambda_orthogonal = cfg.TRAINER.GL_SVDMSE_HER.lambda_orthogonal
        self.alpha = cfg.TRAINER.GL_SVDMSE_HER.alpha
        self.lambda_bal = cfg.TRAINER.GL_SVDMSE_HER.LAMBDA_BAL
        self.topk = cfg.TRAINER.GL_SVDMSE_HER.TOPK
        self.router_type = cfg.TRAINER.GL_SVDMSE_HER.ROUTER_TYPE
